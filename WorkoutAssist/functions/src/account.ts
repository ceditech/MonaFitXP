...
async function exportDocTree(
    ...
    if (collections.length) {
        ...
        for (const col of collections) {
            ...
            node.subcollections[col.id] = await Promise.all(
                docs.docs.map(d => exportDocTree(d.ref)),
            );
        }
    }
    return node;
...
export const deleteAccount = functions.https.onCall(async (_data, context) => {
    ...
    try {
        ...
        throw new functions.https.HttpsError(
            'internal',
            'Could not fully delete your account. Please try again or contact support.',
        );
    }
});
